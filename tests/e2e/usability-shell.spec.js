...
test('catalog filters clear with results and removing favorites preserves keyboard focus @matrix', async ({ page }, info) => {
  await page.goto('/tools')
  const search = page.getByRole('searchbox', { name: 'Werkzeuge suchen', exact: true })
  await search.fill('JSON')
  await expect(page.locator('.catalog-list > li')).toHaveCount(5)
  await page.locator('.catalog-favorite').nth(0).click()
  await page.locator('.catalog-favorite').nth(1).click()
  await page.getByRole('button', { name: 'Filter zurücksetzen', exact: true }).click()
  ...
  const nextName = await page.locator('.catalog-favorite').nth(1).getAttribute('aria-label')
  await page.locator('.catalog-favorite').nth(0).focus()
  await page.keyboard.press('Enter')
  await expect(page.getByRole('button', { name: nextName, exact: true })).toBeFocused()
  await page.keyboard.press('Enter')
  await expect(favorites).toBeFocused()
  await expect(page.locator('.catalog-list > li')).toHaveCount(0)
  await page.getByRole('button', { name: 'English', exact: true }).click()
  await page.getByRole('button', { name: 'Reset filters', exact: true }).click()
  await expect(page.getByRole('searchbox', { name: 'Search tools', exact: true })).toBeFocused()
  expect((await new AxeBuilder({ page }).analyze()).violations).toEqual([])
  expect(await page.evaluate(() => document.documentElement.scrollWidth > window.innerWidth)).toBe(false)
  await page.getByRole('searchbox', { name: 'Search tools', exact: true }).fill('JSON')
  await page.evaluate(() => window.scrollTo(0, 0))
  await page.screenshot({ path: info.outputPath('catalog-clear-filters.png'), fullPage: true })
})
