...
test('crop, clockwise rotation and mirror map original pixels exactly through the worker', async ({ page }) => {
  ...
  await page.getByLabel('Bild auswÃƒÂ¤hlen', { exact: true }).setInputFiles(imageFixture())
  await expect(page.getByText('300 Ãƒâ€” 200 px', { exact: true })).toBeVisible()
  ...
  await page.getByLabel('HÃƒÂ¶he des Ausschnitts').fill('80')
  await page.getByRole('button', { name: 'Ausschnitt anwenden', exact: true }).click()
  await page.getByRole('button', { name: '90Ã‚Â° nach rechts drehen', exact: true }).click()
...
test('PNG keeps alpha, JPEG composites white, and the main-thread fallback exports independently', async ({ page }) => {
  ...
  await page.goto('/image')
  await page.getByLabel('Bild auswÃƒÂ¤hlen', { exact: true }).setInputFiles(imageFixture(120, 80))
...
test('text and watermark gestures create one undo step, Escape cancels, and export excludes controls', async ({ page }) => {
  await page.goto('/image')
  await page.getByLabel('Bild auswÃƒÂ¤hlen', { exact: true }).setInputFiles(imageFixture())
  await page.getByLabel('Textinhalt', { exact: true }).fill('GrÃƒÂ¼sse fÃƒÂ¼r JÃƒÂ¶rg')
  await page.getByRole('button', { name: 'Text hinzufÃƒÂ¼gen', exact: true }).click()
  ...
  const target = page.getByRole('button', { name: 'GrÃƒÂ¼sse fÃƒÂ¼r JÃƒÂ¶rg auswÃƒÂ¤hlen' })
  ...
  await page.getByRole('button', { name: 'RÃƒÂ¼ckgÃƒÂ¤ngig', exact: true }).click()
  ...
  await page.getByLabel('Wasserzeichen hinzufÃƒÂ¼gen').setInputFiles(watermarkFixture())
...
test('EXIF dimensions, invalid files, English dark mobile layout and accessibility pass @matrix', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText('64 Ãƒâ€” 96 px', { exact: true })).toBeVisible()
...
test('portrait preview, numeric crop and pointer resize preserve their selected aspect @matrix', async ({ page }, testInfo) => {
  await page.goto('/image')
  await page.getByLabel('Bild auswÃƒÂ¤hlen', { exact: true }).setInputFiles(imageFixture(600, 1200))
  const stage = page.getByRole('application', { name: 'Bild und Elemente bearbeiten' })
  ...
  await expect(page.getByLabel('HÃƒÂ¶he des Ausschnitts')).toHaveValue('300')
  const resize = page.getByRole('button', { name: 'AusschnittsgrÃƒÂ¶sse ÃƒÂ¤ndern', exact: true })
  ...
  expect(await page.getByLabel('Breite des Ausschnitts').inputValue()).toBe(await page.getByLabel('HÃƒÂ¶he des Ausschnitts').inputValue())
...
test('cancelled and superseded exports release busy state without clearing newer edits', async ({ page }) => {
  ...
  await expect(page.getByText('Bild wird exportiert Ã¢â‚¬Â¦', { exact: true })).toBeVisible()
  await page.getByRole('button', { name: 'Abbrechen', exact: true }).click()
  await expect(page.getByText('Bild wird exportiert Ã¢â‚¬Â¦', { exact: true })).toHaveCount(0)
  ...
  await download
  await expect(page.getByText('Ungespeicherte Ãƒâ€žnderungen', { exact: true })).toBeVisible()
})
...
test('watermark generation rejects late reset results, preserves newer edits and reuses one selection', async ({ page }) => {
  ...
  await page.getByLabel('Wasserzeichen hinzufÃƒÂ¼gen').setInputFiles(watermark)
  await expect.poll(() => page.evaluate(() => typeof window.__releaseWatermark)).toBe('function')
  await page.getByRole('button', { name: 'ZurÃƒÂ¼cksetzen', exact: true }).click()
  ...
  await page.getByRole('button', { name: '90Ã‚Â° nach rechts drehen', exact: true }).click()
  await page.evaluate(() => window.__releaseWatermark())
  await expect(page.getByText('200 Ãƒâ€” 300 px', { exact: true })).toBeVisible()
  await expect(page.locator('.image-element-list button')).toHaveCount(1)
  await page.getByRole('button', { name: 'Wasserzeichen erneut einfÃƒÂ¼gen', exact: true }).click()
...
test('colour, asynchronous watermark, undo and Escape remain separate history steps', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Text hinzufÃƒÂ¼gen', exact: true }).click()
  await page.getByLabel('Wasserzeichen hinzufÃƒÂ¼gen').setInputFiles({ ...watermarkFixture(), name: 'async-colour-watermark.png' })
...
for (const kind of ['watermark', 'text']) test(`image pixels follow a ${kind} gesture before commit and Escape restores them @matrix`, async ({ page }, testInfo) => {
  ...
  await page.getByLabel('Bild auswÃƒÂ¤hlen', { exact: true }).setInputFiles({ name: 'white.png', mimeType: 'image/png', buffer: PNG.sync.write(white) })
  await expect(page.getByText('320 Ãƒâ€” 200 px', { exact: true })).toBeVisible()
  if (kind === 'watermark') await page.getByLabel('Wasserzeichen hinzufÃƒÂ¼gen', { exact: true }).setInputFiles(watermarkFixture())
  else { await page.getByLabel('Textinhalt', { exact: true }).fill('Text in Bewegung'); await page.getByRole('button', { name: 'Text hinzufÃƒÂ¼gen', exact: true }).click() }
  ...
  const target = page.getByRole('button', { name: kind === 'watermark' ? 'local-watermark.png auswÃƒÂ¤hlen' : 'Text in Bewegung auswÃƒÂ¤hlen', exact: true })
...
test('rotated image pixels resize live and navigation discards an unfinished gesture @matrix', async ({ page }, testInfo) => {
  ...
  await page.getByLabel('Bild auswÃƒÂ¤hlen', { exact: true }).setInputFiles(imageFixture())
  await expect(page.getByText('300 Ãƒâ€” 200 px', { exact: true })).toBeVisible()
  await page.getByLabel('Wasserzeichen hinzufÃƒÂ¼gen', { exact: true }).setInputFiles(watermarkFixture())
  await expect(page.getByRole('button', { name: 'local-watermark.png auswÃƒÂ¤hlen', exact: true })).toBeVisible()
  await page.getByRole('button', { name: '90Ã‚Â° nach rechts drehen', exact: true }).click()
  const canvas = page.locator('.image-canvas-stage canvas')
  ...
  const handle = page.getByRole('button', { name: 'ElementgrÃƒÂ¶sse ÃƒÂ¤ndern', exact: true })
  ...
  try {
    ...
    const menu = page.getByRole('button', { name: 'MenÃƒÂ¼ ÃƒÂ¶ffnen', exact: true })
  ...
  const cropHeight = page.getByLabel('HÃ¶he des Ausschnitts', { exact: true })
