...
test('@matrix audio upload and precise fields preserve focus and completed downloads',async({page},info)=>{
 ...
 await open(page,'de')
 const chooser=page.getByRole('button',{name:'Audio auswählen',exact:true})
 ...
 await start.fill('0.45');await start.press('Escape');await expect(start).toHaveValue('0.2');await expect(start).toBeFocused()
 await start.press('Tab');await expect(page.getByLabel('Ende (Sekunden)',{exact:true})).toBeFocused()
 await field(page,'Ende (Sekunden)',.8);await field(page,'Einblenden (Sekunden)',.15);await field(page,'Ausblenden (Sekunden)',.15)
 await page.getByRole('button',{name:'Audio exportieren',exact:true}).click()
 const result=page.getByRole('link',{name:'Audiodatei herunterladen'})
 await expect(result).toBeVisible({timeout:90000})
 const href=await result.getAttribute('href')
 await start.focus();await start.press('Enter');await expect(start).toBeFocused();await start.press('Tab')
 await expect(result).toHaveAttribute('href',href)
 const pending=page.waitForEvent('download');await result.click();const download=await pending,path=info.outputPath('precise-audio.wav');await download.saveAs(path);probe(path,'wav')
 const broken=await page.evaluateHandle(()=>{const transfer=new DataTransfer();transfer.items.add(new File(['not audio'],'broken.mp3',{type:'audio/mpeg'}));return transfer})
 await page.locator('.audio-upload').dispatchEvent('drop',{dataTransfer:broken});await broken.dispose()
 await expect(page.locator('.audio-editor [role=alert]')).toBeVisible({timeout:90000})
 await expect(result).toHaveAttribute('href',href);await expect(start).toHaveValue('0.2');await expect(page.locator('.audio-filename')).toHaveText('sample.wav')
 await page.getByRole('button',{name:'Zurücksetzen',exact:true}).click();await expect(chooser).toBeFocused();await expect(result).toHaveCount(0)
 const transfer=await page.evaluateHandle(bytes=>{const value=new DataTransfer();value.items.add(new File([Uint8Array.from(bytes)],'dropped.wav',{type:'audio/wav'}));return value},Array.from(readFileSync(fixture('wav'))))
 await page.locator('.audio-upload').dispatchEvent('dragenter',{dataTransfer:transfer});await expect(page.locator('.audio-upload')).toHaveClass(/is-dragging/)
 await page.locator('.audio-upload').dispatchEvent('drop',{dataTransfer:transfer});await transfer.dispose()
 await expect(page.locator('.audio-wave')).toBeVisible({timeout:90000});await expect(page.locator('.audio-filename')).toHaveText('dropped.wav')
 await expect(page.locator('.audio-upload')).not.toHaveClass(/is-dragging/)
 await page.getByRole('button',{name:'English',exact:true}).click()
 await expect(page.getByText('Drop another audio file here',{exact:true})).toBeVisible()
 for(const theme of ['light','dark']){
  await page.evaluate(async theme=>{document.documentElement.setAttribute('data-theme',theme);await Promise.all(document.getAnimations().map(animation=>animation.finished.catch(()=>{})))},theme)
  expect((await new AxeBuilder({page}).include('.audio-editor').analyze()).violations).toEqual([])
 }
 expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true)
 expect(requests.filter(request=>new URL(request.url).origin!==server.url||!['GET','HEAD'].includes(request.method)||request.body!==null)).toEqual([]);expect(errors).toEqual([])
 await page.evaluate(async()=>{document.documentElement.setAttribute('data-theme','light');await Promise.all(document.getAnimations().map(animation=>animation.finished.catch(()=>{})))})
 await page.evaluate(()=>scrollTo(0,0));await page.screenshot({path:info.outputPath('audio-upload-fields.png'),fullPage:true,caret:'initial'})
 await page.getByRole('button',{name:'Reset',exact:true}).click()
 await page.evaluate(()=>scrollTo(0,0));await page.screenshot({path:info.outputPath('audio-empty.png'),fullPage:true,caret:'initial'})
})
