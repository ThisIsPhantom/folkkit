...
test('@matrix runs DE and EN core journeys in every supported browser profile', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Inhalt', exact: true }).fill('Folkkit matrix')
  await expect(page.getByRole('button', { name: 'SVG herunterladen', exact: true })).toBeEnabled()

  await page.goto('./workspace?tool=pdf-page-count')
  await page.getByLabel('PDF auswählen', { exact: true }).setInputFiles(fixtureFile('one.pdf', 'application/pdf', onePagePdfBase64))
