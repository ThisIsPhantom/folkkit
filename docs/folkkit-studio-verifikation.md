...
Im nachfolgenden Lauf 34580855677 bestand die Bildeditorprüfung erneut; eine unveränderte native Audio-Vorprüfung in Firefox lieferte innerhalb der Beobachtungsfrist kein Ergebnis. Der Helfer verwendete standardmässiges RAF-Polling für ein nichtvisuelles Medienergebnis. Derselbe Beobachtungsfehler wurde mit unterdrückten Animationsframes gezielt reproduziert. Die Ergebnisbeobachtung verwendet nun ein 100-ms-Intervall. Die native Fünf-Sekunden-Grenze, die äussere Sechs-Sekunden-Grenze und sämtliche Fehlerklassen bleiben bestehen. Neue Browserfälle prüfen fehlende Animationsframes, das Entfernen der Prüfschaltfläche und Ergebnisvariable sowie die unveränderte Ablehnung hängender und verweigerter Starts. Der genaue Auslöser der fehlenden Ergebnisbeobachtung im ursprünglichen CI-Lauf ist durch dessen Log allein nicht bewiesen; entfernt wurde die reproduzierte Abhängigkeit vom Zeichentakt. Die Audio-Anwendungslogik bleibt unverändert.


## Audio-Bedienung vom 12. September 2026

Die Dateiauswahl ist als Ablagefläche mit sichtbarem Auswahlbutton ausgeführt. Drag-and-drop und Tastaturauswahl nutzen denselben vorhandenen Lade- und Validierungspfad. Fehlgeschlagene Ersatzdateien lassen Original, Auswahl und vorhandenen Download erhalten. Zurücksetzen führt den Fokus zum Auswahlbutton. Konventionelle Buttons, Zahlenfelder und Formatwahl sind mindestens 48 Pixel hoch; Rückgängig und Wiederholen bleiben beim Umbruch zusammen.

Zeiteingaben bestätigen mit Enter, verwerfen den Entwurf mit Escape und behalten dabei das Eingabeelement und seinen Fokus. Externe Zustandsänderungen, Undo sowie Dokument- oder Bereichswechsel verwerfen alte Entwürfe. Ein numerisch unveränderter Wert wird vor der Normalisierung erkannt. Dadurch bleiben auch samplegenaue Dauern zwischen zwei Millisekunden, laufende Wiedergabe und fertige Downloads erhalten. Der zusätzliche Zustandsvergleich verhindert wirkungslose History-/Ergebnisänderungen. Engine, Player, Modell, Speichergrenzen und Zeitlimits sind unverändert.

Sieben zusätzliche Komponentenregressionen sichern die Abläufe ab. Fehler und neue Bedienpfade wurden gezielt mit roten und grünen Tests geprüft. Der vollständige Unit-/Vertragssatz bestand mit 994 Tests. Ein neuer Browserfall prüft einen echten Dateidialog, Drop, fehlerhaften Austausch, Enter/Escape/Tab, Reset-Fokus und den erhaltenen Download. Die erzeugte WAV-Auswahl wird unabhängig mit ffprobe und FFmpeg auf Dauer und Fade-Amplituden geprüft. Chromium, Firefox, WebKit und Chromium-Mobile bestanden diesen Ablauf einschliesslich Deutsch/Englisch, hell/dunkel, Axe, Überlaufkontrolle und ausschliesslich Same-Origin GET/HEAD ohne Request-Body. Die vorhandenen HEAD-Anfragen prüfen die selbst gehosteten FFmpeg-Laufzeitdateien. Der Kontrasttest wartet wie die bestehenden Fälle auf abgeschlossene Theme-Übergänge. Ein im unabhängigen Review gefundener Rundungsrandfall wurde mit samplegenauer Dauer reproduziert und behoben.

Der abschliessende Audio-Browsersatz bestand mit 32 Fällen auf dem finalen UI-Build. Er umfasst die bestehenden echten Ein-/Ausgabeformate, Wiedergabegrenzen, Abbruch, Runtime-Isolation und kaltes/warmes Offline-Verhalten. Die gruppierten History-Knöpfe wurden zusätzlich in der Mobilansicht betrachtet und unabhängig nachgeprüft. Lint, Notices-Prüfung und der statische Secret-Scan bestanden.


Die GitHub-Abnahme des Audio-Commits `aa03826` bestand zweimal je 360 Browserfälle bei zwei bestehenden Ausnahmen. Der erste Lauf scheiterte vor einem unveränderten Konverterfall an einem geschlossenen Browserkontext. Dieser Fall und sein Vorgänger bestanden lokal je drei Wiederholungen. Im zweiten Lauf scheiterte die unveränderte Firefox-Griffprüfung an der Treffbarkeit eines kleinen Bildobjekts. Zehn lokale Wiederholungen desselben Falls sowie der anschliessende Bildsatz mit 54 bestandenen Fällen und zwei bestehenden Ausnahmen reproduzierten diesen Fehler nicht. Die genaue Ursache der beiden CI-Signaturen ist damit nicht bewiesen.

Der Bildtest protokolliert bei einem erneuten Treffbarkeitsfehler ausschliesslich feste Kategorien und numerische Geometrie im Runner-Log. Texte, Dateinamen, URLs und DOM-Markup werden nicht ausgegeben. Diese nachgelagerte Diagnose ist auf 1000 ms begrenzt; ein verschwundener Griff liefert einen festen Fallback. Zentrierung, Stabilitätsprüfung, ursprüngliche Hit-Target-Assertion und deren Fristen bleiben unverändert; derselbe Fehler wird erneut geworfen. Zwei neue Browserfälle prüfen eine weiterhin abgewiesene Verdeckung, die tatsächliche Runnerausgabe ohne Fixturetext sowie den begrenzten Fallback. Sie bestehen auch unter den Hosting-Headern, zusammen mit den beiden Griffbedienungsfällen. Der Nachtrag betrifft ausschliesslich Tests und Dokumentation, nicht die Anwendung. Beide im unabhängigen Diagnose-Review gefundenen Befunde wurden korrigiert und nachgeprüft.


Eine anschliessende Messung der gerenderten Bedienelemente zeigte bei den Zahlenfeldern zunächst 41.578125 px statt der deklarierten 48 px. Die spezifischere gemeinsame Studio-Feldregel hatte den lokalen Mindestwert übersteuert. Ein eng auf Audio-Zahlenfelder und Selects begrenzter Mindestwert mit `!important` korrigiert dies, ohne das gemeinsame CSS zu ändern. Der bestehende Audio-Matrixfall prüft nun die tatsächliche BoundingBox-Höhe. Er scheiterte vor der Korrektur an 41.578125 px und bestand danach in allen vier Browserkonfigurationen mit mindestens 48 px. Die ursprüngliche reine Quellprüfung der Feldhöhe wird durch diese Browsermessung korrigiert; ein separater Review bestätigte die begrenzte CSS-Wirkung.


Im nächsten GitHub-Lauf wiederholte sich der Browserkontextfehler direkt nach dem Test zur Bereinigung unbestätigter Inhaltschroniken. Eine separate Fehlersimulation schloss nach den bestehenden Cleanup-Assertions den gemeinsam genutzten Browser und reproduzierte den folgenden `browser.newContext`-Fehler. Dieser Cleanup-Test erhält deshalb einen eigenen Playwright-Worker samt Browser. Dieselbe Simulation besteht damit mit zwei getrennten Prozessen; die normale Seiten-, Kontext-, Launch- und Trace-Verwaltung bleibt bei Playwright. Nachgewiesen ist die Isolation gegen einen geschlossenen Vorgängerbrowser. Die ursprüngliche OS-Ursache des CI-Browserabbruchs bleibt offen.

Der bisherige Dokument-Matrixtest teilte ein Gesamtbudget von 240 Sekunden auf eine Seed-Konvertierung und sechs Formatpaare auf. Nach einem WebKit-Gesamttimeout wird jedes der sechs Paare separat geprüft. Ein Paar erhält 90 Sekunden; die beiden DOCX-Eingangspaare erhalten für den verifizierten Seed und die Zielkonvertierung 150 Sekunden. Die Anwendungsgrenze von 60 Sekunden und die Done-Assertion von 65 Sekunden je Konvertierung bleiben unverändert. Alle 28 bestehenden Assertion-Zeilen und ihre formatabhängigen Prüfungen bleiben erhalten. Die Mobilprüfung erfolgt nun nach jedem Paar. Es gibt keine neuen Skips oder Retries.

Der lokale Shell-/Dokumentsatz besteht mit 53 Fällen, die Prüfung mit Hosting-Headern mit 17 Fällen. Der echte Offline-Neuladefall wurde zusätzlich auf dem aktuellen isolierten Build in allen vier Browserkonfigurationen geprüft. Die unabhängige Codeprüfung bestätigte den getrennten Worker-Digest, die vollständige Paarmenge und die erhaltenen Inhalts-/Netzwerkprüfungen. Dieser Nachtrag verändert ausschliesslich Tests und Dokumentation.
