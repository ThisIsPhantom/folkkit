...
describe('calculator workspace', () => {
  ...
  it('shows discount and surcharge fields, amounts and retained values', () => {
    renderPage()
    fireEvent.change(screen.getByLabelText('Berechnung'), { target: { value: 'discount' } })
    const originalPrice = screen.getByRole('textbox', { name: 'Originalpreis' })
    const discountRate = screen.getByRole('textbox', { name: 'Rabatt (%)' })
    expect(discountRate.labels[0]).toHaveTextContent(/^Rabatt \(%\)$/)
    expect(screen.queryByText('Alle Beträge werden in derselben Währung berechnet.')).not.toBeInTheDocument()
    fireEvent.change(originalPrice, { target: { value: '120' } })
    fireEvent.change(discountRate, { target: { value: '20' } })
    expect(screen.getByTestId('result-changeAmount')).toHaveTextContent('24,00')
    expect(screen.getByTestId('result-finalPrice')).toHaveTextContent('96,00')

    fireEvent.change(screen.getByLabelText('Berechnung'), { target: { value: 'increase' } })
    expect(screen.getByRole('textbox', { name: 'Originalpreis' })).toHaveValue('120')
    expect(screen.getByRole('textbox', { name: 'Aufschlag (%)' })).toHaveValue('20')
    expect(screen.getByTestId('result-changeAmount')).toHaveTextContent('24,00')
    expect(screen.getByTestId('result-finalPrice')).toHaveTextContent('144,00')
  })
  it('fills and clears the price adjustment example and copies its formatted final price', async () => {
    ...
    expect(screen.getByRole('textbox', { name: 'Originalpreis' })).toHaveValue('120')
    expect(screen.getByRole('textbox', { name: 'Rabatt (%)' })).toHaveValue('20')
    fireEvent.click(screen.getByRole('button', { name: 'Endpreis kopieren' }))
    expect(await screen.findByText('Kopiert')).toBeVisible()
    expect(writeText).toHaveBeenCalledWith('96,00')
    fireEvent.click(screen.getByRole('button', { name: 'Leeren' }))
    expect(screen.getByRole('textbox', { name: 'Originalpreis' })).toHaveValue('')
  })
