...
function formulaFor(id, options, t) {
  if (id === 'percent') {
    ...
    if (options.mode === 'discount') return `${t('results.changeAmount')} = ${t('fields.originalPrice')} × ${t('fields.discountRate')} ÷ 100\n${t('results.finalPrice')} = ${t('fields.originalPrice')} − ${t('results.changeAmount')}`
    if (options.mode === 'increase') return `${t('results.changeAmount')} = ${t('fields.originalPrice')} × ${t('fields.surchargeRate')} ÷ 100\n${t('results.finalPrice')} = ${t('fields.originalPrice')} + ${t('results.changeAmount')}`
...
export default function CalculatorPage({ initialCalculator = 'percent', onSelectCalculator }) {
  ...
  const priceAdjustment = active === 'percent' && ['discount', 'increase'].includes(options.mode)
  ...
  const fieldLabel = field => {
    if (priceAdjustment && field === 'base') return t('fields.originalPrice')
    if (priceAdjustment && field === 'rate') return t(`fields.${options.mode === 'discount' ? 'discountRate' : 'surchargeRate'}`)
  ...
  const resultValue = item => typeof item.value === 'string' ? item.value : item.key === 'ratio' ? `${calculation.ratio.width}:${calculation.ratio.height}` : ['loan', 'bmi'].includes(active) || priceAdjustment ? formatAmount(item.value, locale) : formatResult(item.value, locale)
        ...
        <div className="calc-workbench">
          ...
          <div className="calc-workbench-body">
            <form className="calc-form" onSubmit={event => event.preventDefault()} aria-label={t(`calculators.${active}`)}>
              {active === 'percent' && <SelectField label={t('operation')} id="calc-operation" value={options.mode} onChange={value => setOption('mode', value)} entries={entries(['of', 'share', 'change', 'discount', 'increase'], 'percentModes')} />}
              ...
              <div className="calc-input-grid">
                {fields.map(field => {
                  ...
                  return <div className="calc-field" key={field}>
                    <label htmlFor={id}>{fieldLabel(field)}{field === 'rate' && !priceAdjustment && <span className="calc-input-symbol" aria-hidden="true">%</span>}</label>
            ...
            <section className="calc-result" aria-label={t('resultHeading')}>
              ...
              {calculation.status === 'ready' && !calculation.ratio && !['date', 'duration'].includes(active) && <p className="calc-precision">{t(['loan', 'bmi'].includes(active) || priceAdjustment ? 'roundedPrecision' : 'precision')}</p>}
            </section>
          </div>
        </div>
      </div>
