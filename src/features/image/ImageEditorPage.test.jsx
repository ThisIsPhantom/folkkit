...
describe('image editor page', () => {
  ...
  it('rejects foreign opacity starts and ignores foreign pointer completion or cancellation', async () => {
    renderEditor()
    fireEvent.change(screen.getByLabelText('Bild auswählen'), { target: { files: [file()] } })
    await screen.findByText('300 × 200 px')
    fireEvent.change(screen.getByLabelText('Textinhalt'), { target: { value: 'Owner' } })
    fireEvent.click(screen.getByRole('button', { name: 'Text hinzufügen' }))
    const opacity = screen.getByLabelText('Deckkraft')
    expect(fireEvent.pointerDown(opacity, { pointerId: 3, button: 0, isPrimary: false })).toBe(false)
    fireEvent.pointerDown(opacity, { pointerId: 2, button: 0, isPrimary: true })
    fireEvent.change(opacity, { target: { value: '28' } })
    expect(fireEvent.pointerDown(opacity, { pointerId: 3, button: 0, isPrimary: false })).toBe(false)
    expect(fireEvent.pointerDown(opacity, { pointerId: 4, button: 0, isPrimary: true })).toBe(false)
    fireEvent.blur(opacity)
    fireEvent.pointerCancel(opacity, { pointerId: 3 })
    expect(opacity).toHaveValue('28')
    fireEvent.pointerUp(opacity, { pointerId: 3 })
    fireEvent.keyDown(opacity, { key: 'Escape' })
    expect(opacity).toHaveValue('100')
    fireEvent.pointerUp(opacity, { pointerId: 2 })
    fireEvent.click(screen.getByRole('button', { name: 'Rückgängig' }))
    expect(screen.queryByRole('button', { name: 'Text: Owner' })).not.toBeInTheDocument()
  })

  it('ignores late native opacity changes after Escape until the owning pointer ends', async () => {
    renderEditor()
    fireEvent.change(screen.getByLabelText('Bild auswählen'), { target: { files: [file()] } })
    await screen.findByText('300 × 200 px')
    fireEvent.change(screen.getByLabelText('Textinhalt'), { target: { value: 'Late input' } })
    fireEvent.click(screen.getByRole('button', { name: 'Text hinzufügen' }))
    const opacity = screen.getByLabelText('Deckkraft')
    fireEvent.pointerDown(opacity, { pointerId: 2, button: 0, isPrimary: true })
    fireEvent.change(opacity, { target: { value: '28' } })
    ...
    fireEvent.keyDown(opacity, { key: 'ArrowLeft', repeat: true })
    fireEvent.change(opacity, { target: { value: '98' } })
    fireEvent.keyUp(opacity, { key: 'ArrowLeft' })
    expect(opacity).toHaveValue('98')
    fireEvent.click(screen.getByRole('button', { name: 'Rückgängig' }))
    expect(opacity).toHaveValue('100')
    fireEvent.click(screen.getByRole('button', { name: 'Wiederholen' }))
    expect(opacity).toHaveValue('98')
    fireEvent.click(screen.getByRole('button', { name: 'Rückgängig' }))
    fireEvent.click(screen.getByRole('button', { name: 'Rückgängig' }))
    expect(screen.queryByRole('button', { name: 'Text: Late input' })).not.toBeInTheDocument()
  })

  it.each(['pointercancel', 'inactive', 'reset', 'unmount'])('releases opacity capture on %s and prevents a late native edit', async ending => {
    const result = renderEditor()
    fireEvent.change(screen.getByLabelText('Bild auswählen'), { target: { files: [file()] } })
    await screen.findByText('300 × 200 px')
    fireEvent.change(screen.getByLabelText('Textinhalt'), { target: { value: 'Lifecycle' } })
    fireEvent.click(screen.getByRole('button', { name: 'Text hinzufügen' }))
    const opacity = screen.getByLabelText('Deckkraft'), captures = new Set()
    opacity.hasPointerCapture = id => captures.has(id)
    opacity.releasePointerCapture = id => captures.delete(id)
    fireEvent.pointerDown(opacity, { pointerId: 2, button: 0, isPrimary: true })
    // JSDOM has no browser-managed implicit touch capture.
    captures.add(2)
    fireEvent.change(opacity, { target: { value: '28' } })
    if (ending === 'pointercancel') fireEvent.pointerCancel(opacity, { pointerId: 2 })
    if (ending === 'reset') fireEvent.click(screen.getByRole('button', { name: 'Zurücksetzen' }))
    if (ending === 'unmount') result.unmount()
    if (ending === 'inactive') result.rerender(<I18nContext.Provider value={{ locale: 'de', setLocale: vi.fn(), t: (key, vars) => translate({ studioImage: messagesDe }, key, vars) }}><ImageEditorPage active={false} loadFile={loader} renderPreview={async () => {}} /></I18nContext.Provider>)
    expect(captures.size).toBe(0)
    fireEvent.change(opacity, { target: { value: '72' } })
    fireEvent.pointerUp(document, { pointerId: 2 })
    if (ending === 'pointercancel' || ending === 'inactive') {
      expect(screen.getByLabelText('Deckkraft')).toHaveValue('100')
      fireEvent.click(screen.getByRole('button', { name: 'Rückgängig' }))
    }
    expect(screen.queryByRole('button', { name: 'Text: Lifecycle' })).not.toBeInTheDocument()
  })

  it('finishes held opacity before an asynchronous watermark and ignores the remaining touch', async () => {
    const pending = deferred()
    const loadFile = (input, options) => options.role === 'watermark' ? pending.promise : loader(input)
    renderEditor({ loadFile })
    fireEvent.change(screen.getByLabelText('Bild auswählen'), { target: { files: [file()] } })
    await screen.findByText('300 × 200 px')
    fireEvent.change(screen.getByLabelText('Textinhalt'), { target: { value: 'Independent' } })
    fireEvent.click(screen.getByRole('button', { name: 'Text hinzufügen' }))
    fireEvent.change(screen.getByLabelText('Wasserzeichen hinzufügen'), { target: { files: [file('opacity-mark.png')] } })
    fireEvent.pointerDown(screen.getByLabelText('Deckkraft'), { pointerId: 2, button: 0, isPrimary: true })
    fireEvent.change(screen.getByLabelText('Deckkraft'), { target: { value: '28' } })
    pending.resolve({ file: file('opacity-mark.png'), kind: 'png', width: 60, height: 30 })
    await screen.findByRole('button', { name: 'Wasserzeichen: opacity-mark.png' })
    fireEvent.change(screen.getByLabelText('Deckkraft'), { target: { value: '72' } })
    expect(screen.getByLabelText('Deckkraft')).toHaveValue('75')
    fireEvent.click(screen.getByRole('button', { name: 'Rückgängig' }))
    fireEvent.keyDown(document, { key: 'Escape' })
    fireEvent.click(screen.getByRole('button', { name: 'Text: Independent' }))
    expect(screen.getByLabelText('Deckkraft')).toHaveValue('28')
    fireEvent.change(screen.getByLabelText('Deckkraft'), { target: { value: '60' } })
    expect(screen.getByLabelText('Deckkraft')).toHaveValue('28')
    fireEvent.pointerUp(document, { pointerId: 2 })
    fireEvent.click(screen.getByRole('button', { name: 'Rückgängig' }))
    expect(screen.getByLabelText('Deckkraft')).toHaveValue('100')
